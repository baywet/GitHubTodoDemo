...
class GitHubAccessTokenProvider(AccessTokenProvider):
    ...
    async def get_authorization_token(
        ...
        print(
            f"Please go to {device_code_response.verification_uri} and enter the code {device_code_response.user_code} to authenticate."
        )
    ...
    async def poll_for_authorization_token(
            self, device_code_response: GitHubDeviceCodeResponse) -> str:
    ...
    async def get_device_code(self) -> GitHubDeviceCodeResponse:
        ...
        if response.status_code == 200:
            json_response = response.json()
            return GitHubDeviceCodeResponse(json_response["device_code"],
